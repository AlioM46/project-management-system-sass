...
export function ChatMessageArea({
    ...
    const mentionCandidatesSource = useMemo(() => {
        // Mentions are only allowed in project or group conversations (never direct 1-on-1 DMs)
        if (!conversation?.participants || isDirect) return [];
        return conversation.participants
            .filter((p: Participant) => p.is_active && p.user)
            .map((p: Participant) => ({
                id: p.user.id,
                name: p.user.name,
    ...
    }, [conversation?.participants, conversation?.type]);
    ...
    const {
        ...
        handleInputClick: handleMentionClick,
        handleInputKeyUp: handleMentionKeyUp,
        handleMentionSelect,
        closeMention,
    } = useMentions({
        message: inputText,
        setMessage: onInputTextChange,
        candidates: mentionCandidatesSource,
    ...
    useEffect(() => {
        if (conversation?.id) {
            markConversationAsReadApi(conversation.id);
        }
    }, [conversation?.id]);

    const MessageReceipt = ({ isMe, status }: { isMe: boolean; status?: string }) => {
        if (!isMe || !status || status === "none") return null;
    ...
    return (
        <div className="flex-1 flex flex-col bg-zinc-50/50 dark:bg-[#050505]">
                                    ...
                                    <div className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                                        ...
                                        <div
                                            className={`px-4 py-2.5 text-sm leading-relaxed shadow-sm transition-all relative ${msg.isDeleted
                                                ...
                                                : "bg-white dark:bg-white/5 text-zinc-800 dark:text-zinc-200 border border-zinc-200/80 dark:border-white/10 rounded-2xl"
                                                }`}
                                        >
                                            {msg.isDeleted ? (
                                                ...
                                                <>
                                                    ...
                                                    {msg.parent && (
                                                        <div
                                                            ...
                                                            className="mb-2 p-2.5 rounded-xl border-l-[4px] text-xs cursor-pointer transition-all shadow-sm bg-zinc-100 dark:bg-white/10 border-blue-500 text-zinc-800 dark:text-zinc-200 hover:bg-zinc-200/80 dark:hover:bg-white/15"
                                                        >
                                                            <div className="flex items-center justify-between gap-2 font-bold text-[11px] text-blue-600 dark:text-blue-400">
                                                                <span>{msg.parent.sender?.name || "User"}</span>
                                                                <Reply className="h-3 w-3 opacity-80" />
                                                            </div>
                                                            <p
                                                                className="line-clamp-2 mt-0.5 font-normal text-zinc-600 dark:text-zinc-300"
                                                                dangerouslySetInnerHTML={{ __html: msg.parent.FormattedBody || msg.parent.body }}
                                                            />
                                                        </div>
                                                    )}
                                                    ...
                                                    {(msg.FormattedBody || msg.body) && (
                                                        <div className="flex items-end justify-between gap-2">
                                                            <div
                                                                className="text-sm leading-6 text-zinc-700 dark:text-zinc-200 font-normal"
                                                                dangerouslySetInnerHTML={{ __html: msg.FormattedBody || msg.body }}
                                                            />
                                                            {msg.is_starred && (
                                                    ...
                                                    {/* Timestamp & Read Receipt Checkmarks inside message bubble */}
                                                    <div className="flex items-center justify-end gap-1 mt-1.5 select-none">
                                                        {msg.isEdited && (
                                                            <span className="text-[9px] text-zinc-400 dark:text-zinc-400 italic mr-0.5 opacity-75">
                                                                edited
                                                            </span>
                                                        )}
                                                        <span className="text-[10px] font-medium text-zinc-400 dark:text-zinc-400 opacity-80">
                                                            {formatTime(msg.created_at)}
                                                        </span>
                                                        <MessageReceipt isMe={isMe} status={msg.status} />
                                                    </div>
                                                </>
                                            )}
                                        </div>
            ...
            <div className="px-5 pb-4 pt-2 shrink-0 relative">
                {/* Floating @Mention Suggestions Popup */}
                {isMentionOpen && mentionCandidates.length > 0 && (
                    <div className="absolute bottom-[calc(100%+8px)] left-5 z-40 w-72 max-h-60 overflow-y-auto rounded-2xl border border-zinc-200 bg-white p-1.5 shadow-xl dark:border-white/10 dark:bg-[#121212] animate-in fade-in slide-in-from-bottom-2">
                        ...
                        {mentionCandidates.map((candidate) => (
                            <button
                                key={candidate.id}
                                type="button"
                                className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-left text-sm hover:bg-zinc-100 dark:hover:bg-white/5 transition-colors"
                                onClick={() => handleMentionSelect(candidate.username)}
                            >
                                <UserAvatar
                                    name={candidate.name}
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="truncate font-medium text-zinc-900 dark:text-white text-xs">
                                        {candidate.name}
                                    </div>
                ...
                {conversation?.is_blocked_by_me ? (
                    ...
                    <div className="flex items-end gap-2 bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-2xl px-4 py-2 shadow-sm focus-within:border-blue-500/50 focus-within:shadow-md transition-all">
                        ...
                        <textarea
                            ...
                            placeholder={isDirect ? `Type a message...` : `Type a message or type @ to mention...`}
                            rows={1}
