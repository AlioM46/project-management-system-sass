...
import { deleteMessageForAll, deleteMessageForMe, getConversations, getMessages, sendMessage, toggleMessageReaction, updateMessage, toggleStarMessage, blockUser, unblockUser, markConversationAsReadApi, markAsDeliveredApi } from "@/features/chat/api/chat.api";
...
export default function ChatPage() {
    ...
    const handleIncomingMessage = useCallback(
        async (newMessage: Message) => {
            const senderId = Number(newMessage.user_id || newMessage.sender?.id);
            const isFromOtherUser = senderId !== Number(currentUserId);

            if (isFromOtherUser && newMessage.id) {
                // 1. Always ACK Delivery since recipient device received the message payload!
                markAsDeliveredApi(newMessage.conversation_id, newMessage.id);

                // 2. If the recipient is ALREADY inside this active conversation room, ALSO mark as READ!
                if (Number(newMessage.conversation_id) === Number(activeConversationId)) {
                    markConversationAsReadApi(newMessage.conversation_id);
                }
            }
    ...
    const handleMessageDelivered = useCallback(
        (event: { conversationId: number; deliveredAt: string }) => {
            console.log("Realtime Message Delivered Event: ", event);
            setMessages((prev) =>
                prev.map((msg) => {
                    const msgUserId = Number(msg.user_id || msg.sender?.id);
                    if (msgUserId === Number(currentUserId) && !msg.delivered_at) {
                        return { ...msg, delivered_at: event.deliveredAt, status: "delivered" };
                    }
                    return msg;
                })
            );
    ...
    const handleMessageRead = useCallback(
        (event: { conversationId: number; userId: number; readAt: string }) => {
            console.log("Realtime Message Read Event: ", event);
            setMessages((prev) =>
                prev.map((msg) => {
                    const msgUserId = Number(msg.user_id || msg.sender?.id);
                    if (msgUserId === Number(currentUserId) && new Date(msg.created_at) <= new Date(event.readAt)) {
