...
export default function TrashPage() {
    ...
    const handleForceDelete = async () => {
        ...
        try {
            ...
            setItems((prev) => prev.filter((i) => !(i.type === itemToPurge.type && i.id === itemToPurge.id)));
            setItemToPurge(null);
        } catch (error) {
                    ...
                    <div className="flex items-start gap-3.5">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-0.5 max-w-xl">
                                Deleted projects and tasks remain in Trash for 30 days before being permanently purged. You can restore items or delete them permanently at any time.
                            </p>
                        </div>
                    </div>
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-white/5 text-center px-4">
                    ...
                    <h4 className="text-base font-semibold text-zinc-900 dark:text-white">
                        {searchQuery ? "No matching items found" : "Trash is completely empty"}
                    </h4>
                ...
                <div className="divide-y divide-zinc-200 dark:divide-white/5 bg-white dark:bg-zinc-900/40 rounded-2xl border border-zinc-200 dark:border-white/5 overflow-hidden shadow-sm">
                    {filteredItems.map((item) => {
                        ...
                        const isRestoring = isActionLoading === `restore-${item.type}-${item.id}`;
                            ...
                            >
                                <div className="flex items-start gap-3.5 min-w-0">
                                    <div className={`p-2.5 rounded-xl shrink-0 ${
                                        ...
                                        {isTask ? <CheckSquare className="w-5 h-5" /> : <FolderKanban className="w-5 h-5" />}
                                    </div>
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {item.project_name && (
                                                <span className="text-xs text-zinc-500 dark:text-zinc-400">
                                                    in <span className="font-medium text-zinc-700 dark:text-zinc-300">{item.project_name}</span>
                                                </span>
                                            )}
                                        </div>
                        ...
                        <div className="flex items-start gap-3.5">
                            ...
                            <div>
                                <h3 className="text-base font-bold text-zinc-900 dark:text-white">
                                    Permanently Delete {itemToPurge.type === "project" ? "Project" : "Task"}?
                                </h3>
                                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                                    Are you sure you want to permanently delete <strong className="text-zinc-900 dark:text-white">{itemToPurge.title}</strong>? This action cannot be undone.
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
                        <div className="flex items-start gap-3.5">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                                    This will permanently destroy all <strong className="text-zinc-900 dark:text-white">{items.length} trashed items</strong> in this workspace. All data and attachments will be deleted immediately.
                                </p>
                            </div>
                        </div>
